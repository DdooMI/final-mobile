...
const PaymentPage = () => {
  ...
  // Fake user data
  const fakeUser = { uid: "12345", name: "John Doe" };
  const fakeRole = "client";
  const fakeBalance = 100.0;

  const navigation = useNavigation();

  const initialize = () => {
    if (fakeRole === "designer") {
  ...
  const handleSubmit = () => {
    ...
    if (operationType === "withdraw" && numAmount > fakeBalance) {
      setError("Insufficient balance for withdrawal");
      return;
    }
    setIsProcessing(true);
    setTimeout(() => handlePayPalSuccess(), 2000);
  };
  ...
  return (
    <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.formContainer}>
        ...
        >
          <Text style={styles.backButtonText}>Back to profile</Text> {/* Updated text for clarity */}
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.summaryContainer}>
        ...
        <View style={styles.balanceCard}>
          ...
          <Text style={styles.balanceAmount}>${fakeBalance.toFixed(2)}</Text>
